...
import {
  ...
  schedules,         // Import schedules table schema
  insertScheduleSchema, // Import Zod schema for inserting schedules
  updateScheduleSchema  // Import Zod schema for updating schedules
} from "@shared/schema";
import { z } from "zod";
...
export function registerRoutes(app: Express): Server {
  ...
  app.get("/api/schedules", async (req, res) => {
    ...
    try {
      // Assuming schedules are global for now, or implicitly tied to user via test plans if that link existed.
      // Add .where(eq(schedules.userId, req.user.id)) if userId is added to schedules table.
  ...
  app.post("/api/schedules", async (req, res) => {
    ...
    // Add 'id' to the Zod schema for validation if client is expected to provide it.
    // For server-generated ID using uuid:
    const schedulePayloadSchema = insertScheduleSchema.omit({ id: true }); // ID will be generated by server

    const parseResult = schedulePayloadSchema.safeParse(req.body);
    if (!parseResult.success) {
    ...
    try {
      ...
      const scheduleId = uuidv4(); // Generate new UUID for the schedule

      // Convert nextRunAt to number (Unix timestamp in seconds) if it's a Date object
      ...
      const createdSchedule = await db
        ...
        .values({
          ...
          // createdAt is handled by default in schema (strftime('%s', 'now'))
          // updatedAt can be set to current time as well, or handled by application logic if preferred for inserts
      ...
      if (createdSchedule.length === 0) {
        ...
        return res.status(500).json({ error: "Failed to create schedule, returned no data." });
      }
  ...
  app.put("/api/schedules/:id", async (req, res) => {
    ...
    // (Add check: if schedules become user-specific, ensure req.user.id matches schedule owner)
    ...
    try {
      ...
      const updatedSchedule = await db
        ...
        .set({
          ...updates,
          nextRunAt: nextRunAtTimestamp, // Use the processed timestamp
          updatedAt: Math.floor(Date.now() / 1000), // Set updatedAt to current time as Unix timestamp
        })
        .where(eq(schedules.id, scheduleId))
        // Add .where(and(eq(schedules.id, scheduleId), eq(schedules.userId, req.user.id))) if user-specific
        .returning();
