...
export class BrowserPool {
  ...
  public async acquire(browserType: 'chromium' | 'firefox' | 'webkit' = DEFAULT_BROWSER, headless: boolean = DEFAULT_HEADLESS): Promise<Browser> {
    ...
    const availableItem = this.pool.find(item => 
      ...
      this.isHeadlessMatch(item, headless)
    );
  ...
  // Helper to check headless state based on our saved PoolItem definition
  private isHeadlessMatch(item: PoolItem, targetHeadless: boolean): boolean {
      return item.headless === targetHeadless;
  }
}
