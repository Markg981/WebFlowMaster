...
export async function processTestPlanJob(
  ...
  // Bolt Optimization: Batch fetch UI and API tests to resolve N+1 query bottleneck
  const uiTestIds = selectedTestsLinks.filter(l => l.testId && l.testType === 'ui').map(l => l.testId!);
  const apiTestIds = selectedTestsLinks.filter(l => l.apiTestId && l.testType === 'api').map(l => l.apiTestId!);
  ...
  if (uiTestIds.length > 0) {
    ...
    for (const t of uiTests) uiTestsMap.set(t.id, t);
  }
  ...
  if (apiTestIds.length > 0) {
    ...
    for (const t of apiTests) apiTestsMap.set(t.id, t);
  }
