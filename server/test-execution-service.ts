...
export async function runTestPlan(
  ...
  // Bolt ⚡: Pre-fetch UI and API tests to avoid N+1 query bottlenecks
  const uiTestIds = selectedTestsLinks.filter(link => link.testType === 'ui' && link.testId != null).map(link => link.testId!);
  const apiTestIds = selectedTestsLinks.filter(link => link.testType === 'api' && link.apiTestId != null).map(link => link.apiTestId!);
