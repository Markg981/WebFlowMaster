...
export async function processTestPlanJob(
  ...
  // ⚡ Bolt: Replaced multiple O(N) array .filter() passes with a single O(N) loop
  // This reduces memory allocations and CPU cycles when calculating test metrics
  ...
  for (const r of finalDetailedResults) {
    ...
    else if (r.status === 'Error') hasErrorTests = true;
  }
  ...
  } else if (calculatedFailedTests > 0 || hasErrorTests) {
