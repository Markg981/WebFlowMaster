...
// Fetch all test plans (full view for management page)
export const fetchTestPlansAPI = async (): Promise<TestPlan[]> => {
...
interface RunTestPlanResponse {
  success: boolean;
  data?: any; // Avoiding deep type deps for now if not exported
  error?: string;
}
