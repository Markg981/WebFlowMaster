...
export const userSettings = sqliteTable("user_settings", {
  ...
  playwrightHeadless: integer("playwright_headless", { mode: "boolean" })
    .default(true)
    .notNull(),
  playwrightDefaultTimeout: integer("playwright_default_timeout")
    .default(30000)
    .notNull(),
...
export const detectedElements = sqliteTable("detected_elements", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  testId: integer("test_id").references(() => tests.id, { onDelete: "cascade" }),
  elementId: text("element_id"), // ID from DOM traversal
  selector: text("selector").notNull(),
  originalSelector: text("original_selector"), // In case of healing
  type: text("type"),
  text: text("text"),
  tag: text("tag"),
...
export const testPlans = sqliteTable("test_plans", {
  ...
  visualTestingEnabled: integer("visual_testing_enabled", {
    mode: "boolean",
  }).default(false),
  pageLoadTimeout: integer("page_load_timeout").default(30000),
  elementTimeout: integer("element_timeout").default(30000),
  onMajorStepFailure: text("on_major_step_failure").default(
    "abort_and_run_next_test_case",
  ),
  onAbortedTestCase: text("on_aborted_test_case").default(
    "delete_cookies_and_reuse_session",
  ),
  onTestSuitePreRequisiteFailure: text(
    "on_test_suite_pre_requisite_failure",
  ).default("stop_execution"),
  onTestCasePreRequisiteFailure: text(
    "on_test_case_pre_requisite_failure",
  ).default("stop_execution"),
  onTestStepPreRequisiteFailure: text(
...
export const testPlanSchedules = sqliteTable("test_plan_schedules", {
  ...
  notificationConfigOverride: text("notification_config_override", {
    mode: "json",
  }),
  executionParameters: text("execution_parameters", { mode: "json" }),
  isActive: integer("is_active", { mode: "boolean" }).default(true).notNull(),
  retryOnFailure: text("retry_on_failure", { enum: ["none", "once", "twice"] })
    .default("none")
    .notNull(),
  createdAt: integer("created_at", { mode: "timestamp" })
    .default(sql`(strftime('%s', 'now'))`)
...
export const testPlanExecutions = sqliteTable("test_plan_executions", {
  id: text("id").primaryKey(),
  scheduleId: text("schedule_id").references(() => testPlanSchedules.id, {
    onDelete: "set null",
  }),
  testPlanId: text("test_plan_id")
    .notNull()
  ...
  environment: text("environment"),
  browsers: text("browsers", { mode: "json" }),
  triggeredBy: text("triggered_by", { enum: ["scheduled", "manual", "api"] })
    .notNull()
    .default("manual"),
...
export const reportTestCaseResults = sqliteTable("report_test_case_results", {
  id: text("id").primaryKey(),
  testPlanExecutionId: text("test_plan_execution_id")
    .notNull()
    .references(() => testPlanExecutions.id, { onDelete: "cascade" }),
  uiTestId: integer("ui_test_id").references(() => tests.id, {
    onDelete: "set null",
  }),
  apiTestId: integer("api_test_id").references(() => apiTests.id, {
    onDelete: "set null",
  }),
...
export type InsertReportTestCaseResult =
  typeof reportTestCaseResults.$inferInsert;
